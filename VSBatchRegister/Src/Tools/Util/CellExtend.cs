...
public static class CellExtend
{
    ...
    /// <param name="places">小数点后X位数、默认2位</param>
    /// <returns></returns>
    public static string DecimalToString(this decimal? value, int places = 2)
    {
        // 处理特殊值
        if (value == null)
        {
    ...
    public static bool ContainsNumericInCells(IRow row, int[] selectedCellIndices, int minimumRequiredConditions = 1)
    {
        ...
        return ContainsNumericInCells(minimumRequiredConditions, cells);
    }
    ...
    public static bool ContainsValueInCells(IRow row, int[] selectedCellIndices, int minimumRequiredConditions = 1)
    {
        ...
        return ContainsValueInCells(minimumRequiredConditions, cells);
    }
